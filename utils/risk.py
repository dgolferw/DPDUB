...
def rotate_losers_to_cash(positions, signals, cash, portfolio_value, exclude=None):
    ...
    eligible = {sym: pos for sym, pos in positions.items()
                if sym not in exclude and float(pos.qty) >= MIN_QTY}

    if len(eligible) > config.MAX_POSITIONS:
        excess_count = len(eligible) - config.MAX_POSITIONS
        candidates = sorted(
            [(sym, float(pos.unrealized_plpc), float(pos.market_value))
             for sym, pos in eligible.items()
