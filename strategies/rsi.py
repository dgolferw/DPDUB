...
class RSIMeanReversion(BaseStrategy):
    ...
    description = "Tier-based RSI with volume, momentum, gap, regime filters."
    ...
    def _get_tier(self, sym):
        ...
        elif sym in config.TIER2: return 2
        return 3
    ...
    def _volume_confirmed(self, df):
        if "volume" not in df.columns or len(df) < config.VOLUME_MA_DAYS: return True
    ...
    def _get_order_fraction(self, sym, signal):
        ...
        if tier == 1: return config.ORDER_FRACTION_TIER1_STRONG if signal == "strong_buy" else config.ORDER_FRACTION_TIER1_NORMAL
        elif tier == 2: return config.ORDER_FRACTION_TIER2_STRONG if signal == "strong_buy" else config.ORDER_FRACTION_TIER2_NORMAL
        ...
        for sym, df in bars.items():
            ...
            if regime == "bear":
                if tier == 3 and sym != "SQQQ":
                    signals[sym] = ("hold", config.ORDER_FRACTION, trail); continue
                if tier in (1,2) and rsi >= config.STRONG_OVERSOLD:
                    signals[sym] = ("hold", config.ORDER_FRACTION, trail); continue
            if gap: signals[sym] = ("hold", config.ORDER_FRACTION, trail)
            elif rsi < config.STRONG_OVERSOLD and vol_ok: signals[sym] = ("strong_buy", self._get_order_fraction(sym,"strong_buy"), trail)
            elif rsi < config.NORMAL_OVERSOLD and above_ma and vol_ok: signals[sym] = ("buy", self._get_order_fraction(sym,"buy"), trail)
            elif rsi < config.WEAK_OVERSOLD and above_ma and vol_ok and tier in (1,2): signals[sym] = ("buy", self._get_order_fraction(sym,"buy"), trail)
            elif rsi > sell_rsi: signals[sym] = ("sell", config.ORDER_FRACTION, trail)
            else: signals[sym] = ("hold", config.ORDER_FRACTION, trail)
